...
public abstract class PokemonEffect extends Effect implements Serializable
{
	...
	private static class LeechSeed extends PokemonEffect implements EndTurnEffect, RapidSpinRelease, PassableEffect
	{
		...
		{
			return !(victim.isType(Type.GRASS) || victim.hasEffect("LeechSeed"));
		}
		...
		{
			if (victim.isType(Type.GRASS)) return "It doesn't affect "+victim.getName()+"!";
			if (victim.hasEffect("LeechSeed")) return victim.getName()+" is already seeded!";
			return super.getFailMessage(user, victim, team);
		}
	}
	...
	private static class Mist extends PokemonEffect implements DefogRelease, StatProtectingEffect
	{
		private static final long serialVersionUID = 1L;
		public Mist()
		{
	...
	private static class Bide extends PokemonEffect implements ForceMoveEffect, IntegerCondition, EndTurnEffect
	{
		private static final long serialVersionUID = 1L;
		private Move move;
		private int turns;
	...
	private static class PerishSong extends PokemonEffect implements PassableEffect, EndTurnEffect
	{
		...
		public String getFailMessage(ActivePokemon user, ActivePokemon victim, boolean team)
		{
			if (victim.hasAbility("Soundproof")) return victim.getName()+"'s Soundproof makes it immune to sound based moves!";
			return super.getFailMessage(user, victim, team);
		}
	}
