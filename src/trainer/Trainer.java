...
//	protected boolean isBTTSROUI;
	...
	public void setFront(int index)
	{
		...
		if (front().getAbility() instanceof SwitchOutEffect) ((SwitchOutEffect)front().getAbility()).switchOut(front());
	...
	public void resetUsed()
	{
		for (int i = 0; i < team.size(); i++)
		{
			if (i == frontIndex) team.get(i).getAttributes().setUsed(true);
			else team.get(i).getAttributes().setUsed(false);
		}
	}
	...
	public void switchToRandom()
	{
		...
		if (valid.size() == 0) Global.error("You shouldn't be switching when you have nothing to switch to!");
