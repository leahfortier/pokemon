...
public class PC implements Serializable
{
	...
	public static int BOX_WIDTH = 6;
	public static int BOX_HEIGHT = 6;
	
	private int currBox, numPokemon;
	...
	public void addBox()
	{
		if (boxes == null) Global.error("Error adding box, boxes is null.");
		boxes.add(new Box());
	}
	
	public void addBoxes(int n)
	{
		for (int i = 0; i < n; i++) addBox();
	}
	...
	public ActivePokemon[][] getBoxPokemon(int box)
	{
	...
	public void depositPokemon(CharacterData player, ActivePokemon p, int boxNum, int x, int y)
	{
		if (!player.canDeposit(p) || !inBounds(x, y) || getBoxPokemon()[x][y] != null) return;
	...
	private void boxPartySwap(int boxNum, int i, int j, CharacterData player, int partyIndex)
	{
		...
		else
		{
			...
			if (boxPokemon.isEgg()) eggs++;
			if (partyPokemon.isEgg()) eggs--;
			
			// If swapping these Pokemon will make the player only have eggs, we can't do it.
			if (eggs == player.getTeam().size()) return;
	...
	{
		if (i < 0 || i >= BOX_HEIGHT || j < 0 || j >= BOX_WIDTH) return false;
		return true;
	}
